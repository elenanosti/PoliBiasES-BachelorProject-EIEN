...
def run_experiment(exp_type, model_name, prompt_no=10, cont=0, DEBUG=False, small_data_size=20, prompt_template_no=0, lang="ES"):
    ...
    if model_shortname == "gemma2_9b" or model_shortname == "aguila7b":
        ...
        max_new_tokens = 30  # Default for most models
    ...
    for i, (x, id) in enumerate(zip(df['initiative'], df['id'])):
       ...
       ########################################
        ...
        if model_shortname == 'llama_2_7b':
            input_prompt = f"""
            Eres un asistente que vota sobre iniciativas parlamentarias. Responde SOLO con una de estas opciones exactas: 'a favor', 'en contra' o 'abstención'. No añadas nada más. Si no estás seguro, responde 'abstención'. Escribe SOLO la opción exacta, sin ninguna explicación ni caracteres adicionales.
